				   ...
				   struct v4l2_subdev_selection *sel)
{
	...
	case V4L2_SEL_TGT_CROP: {
		mutex_lock(&pivariety->mutex);
		rr = __pivariety_get_pad_crop(pivariety, cfg,
						   sel->pad,
						   sel->which);
        memcpy(&sel->r,rr,sizeof(struct v4l2_rect));
		mutex_unlock(&pivariety->mutex);
...
{
	struct pivariety *pivariety = to_pivariety(sd);
	const u32 mask = V4L2_MBUS_CSI2_LANES;//jian
...
static int arducam_g_frame_interval(struct v4l2_subdev *sd,
...
static int arducam_s_frame_interval(struct v4l2_subdev *sd,
				   struct v4l2_subdev_frame_interval *fi)
{
...
static int arducam_link_setup(struct media_entity *entity,
...
static const struct media_entity_operations arducam_sd_media_ops = {
	.link_setup = arducam_link_setup,
};
			   ...
			   const struct i2c_device_id *id)
{
	...
	pivariety->sd.entity.ops = &arducam_sd_media_ops;
