...
static unsigned int DbgLevel =0xffff;// DBG_MODULE|DBG_SUSP;
...
static void ProcessReport(unsigned char *buf, struct _egalax_i2c *p_egalax_i2c)
{
	...
	while(index<MAX_POINT_PER_PACKET)
	{
		...
		pContactBuf[RecvPtsCnt].X = ((x*1920*100)/MAX_RESOLUTION)/100;
		pContactBuf[RecvPtsCnt].Y = ((y*1080*100)/MAX_RESOLUTION)/100;
...
static struct input_dev * allocate_Input_Dev(void)
{
	...
	input_set_abs_params(pInputDev, ABS_X, 0, MAX_X_RESOLUTION, 0, 0);
	input_set_abs_params(pInputDev, ABS_Y, 0, MAX_Y_RESOLUTION, 0, 0);
#endif
	...
	#endif
	input_set_abs_params(pInputDev, ABS_MT_POSITION_X, 0, MAX_X_RESOLUTION, 0, 0);
	input_set_abs_params(pInputDev, ABS_MT_POSITION_Y, 0, MAX_Y_RESOLUTION, 0, 0);
	input_set_abs_params(pInputDev, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
#else
	input_set_abs_params(pInputDev, ABS_MT_POSITION_X, 0, MAX_X_RESOLUTION, 0, 0);
	input_set_abs_params(pInputDev, ABS_MT_POSITION_Y, 0, MAX_Y_RESOLUTION, 0, 0);
	input_set_abs_params(pInputDev, ABS_MT_WIDTH_MAJOR, 0, MAX_X_RESOLUTION, 0, 0); //Size
...
static int __devinit egalax_i2c_probe(struct i2c_client *client, const struct i2c_device_id *idp)
{
	...
	p_egalax_i2c_dev->enable_gpio = of_get_named_gpio(devnode, "enable-gpios", 0);
	if( gpio_is_valid(p_egalax_i2c_dev->enable_gpio) )
    {
	     gpio_direction_output(p_egalax_i2c_dev->enable_gpio, 1);
	     mdelay(20);
    }
#else
